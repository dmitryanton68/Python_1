...
class Game:
    ...
    def list_generate(self):
        lst = [i for i in range(1, self._MAX_NUMBER + 1)]
        for _ in range(15):
            lst.append('-')
        shuffle(lst)
        return lst

    def card_generate(self, ind, lst_, round):
        if ind == 'player':
            lst_player = lst_
    ...
    def choise_number(self, lst):
        number = lst.pop()
        return number

    def play_cycle(self):
        lst_player = self.list_generate()
        lst_machine = self.list_generate()
        choice_lst = self.list_generate()
        for number_of_round in range(1, 8):
            print(f"--------- Раунд номер {number_of_round} ------------")
            card_player = self.card_generate('player', lst_player, number_of_round)
            card_machine = self.card_generate('machine', lst_machine, number_of_round)
            for i in range(15):
                num = self.choise_number(choice_lst)
                ...
                if player_choise == 'y':
                    if num in card_player:
                        self.choise_number(card_player)
                        continue
                    else:
